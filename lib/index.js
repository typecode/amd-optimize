(function() {
  ...
  collectModules = function(modules, omitInline) {
  ...
  defaultLoader = function(fileBuffer, options) {
    return function(name, callback, asPlainFile) {
      var addJs, file;
      addJs = (!asPlainFile) && name.indexOf('.js') === -1 ? '.js' : '';
  ...
  module.exports = rjs = function(entryModuleData, options) {
      ...
      return async.waterfall([
        function(callback) {
          ...
          if (entryModuleData === null) {
            moduleNames = fileBuffer.map(function(file) {
              return file.path.replace(file.base, '').replace('.js', '');
            });
          } else {
            moduleNames = _.isArray(entryModuleData) ? entryModuleData : [entryModuleData];
          }
          return async.map(moduleNames, function(moduleName, callback) {
